...
- [x] 1.1 Give the train and station layers `pickable: true`, and set the cursor and picking radius
...
- [x] 1.2 Leave the track layers **not** pickable. They are everywhere near a line and would answer
      instead of the train standing on them. Verify hovering a line between stations identifies
      nothing.
- [x] 1.3 Give train instances an identity. They currently carry position, orientation and colour
...
- [x] 1.4 Confirm a train standing at a platform wins the pick over the marker beneath it. This
...
- [x] 2.1 Show the line and destination for a train, the name for a station, following the pointer.
...
- [x] 2.2 Suppress hover for touch, or a tap flashes a description before opening the card. Verify
      by tapping.

## 3. The train card

- [x] 3.1 Show the line, where the train is going, where it started and at what time. The start time
      is the departure second within that train's own service day, so a train that left at 23:50
      yesterday must read 23:50.
- [x] 3.2 Show what it is doing: standing at a station with time until it leaves, or approaching one
...
- [x] 3.3 List the stops still to come with their scheduled times, computed from the trip's start
...
- [x] 3.4 Add a duration formatter to the UI formatters, with a test. It does not exist in the port.

## 4. The station card

- [x] 4.1 Show the next departures per direction, labelled by destination. Resolve the stop's index
      **within each direction separately** — direction 1's stop list is direction 0's reversed, so
      ...
      station.
- [x] 4.2 Do not advertise departures at a terminal in the direction nothing leaves in. Verify with
      a test at both ends of a line.
- [x] 4.3 Where a train already running will call at this station, derive the time from that train's
...
- [x] 4.4 Show both a clock time and a wait, and say plainly when there are no more trains today.

## 5. Follow

- [x] 5.1 Move the camera every frame with a jump, not an animation — an animation started each
...
- [x] 5.2 Centre on the train's **drawn** position, which carries the keep-left shift and the
...
- [x] 5.3 Stop following when the viewer drags the map, using the map's own signal that a person
...
- [x] 5.4 Stop following when the train's trip ends or it is no longer shown, including because its
      line was hidden.

## 6. The lines panel

- [x] 6.1 One row per line: colour, name, running count, and a switch. Native `<details>` and native
      checkboxes, for the reason the existing components give.
- [x] 6.2 Hiding a line hides its track, its stations and its trains, by filtering the layers' data
...
- [x] 6.3 Rebuild the line and station layers only when the hidden set changes, following the
      existing pattern for rebuilding on camera change.
- [x] 6.4 Clear the selection when its line is hidden — including for a train whose trip has already
      ended, which means carrying the line id alongside the selection.
- [x] 6.5 Write the running counts to the DOM **through refs**, a few times a second, following the
      clock readout. A `setState` four times a second is still a React render driven by the frame
      loop.

## 7. Selection that does not lie

- [x] 7.1 Hold the selection as a train id or a stop id, with the line id alongside. Not an array
      index — those are rebuilt whenever the camera changes.
- [x] 7.2 Recognise the same train under a changed identity. A train's id is composed from its day
...
- [x] 7.3 Distinguish "this trip has ended" from "I can no longer find this", and say the second
      honestly rather than dressing it as the first. Verify by selecting a train and forcing a
      different timetable.

## 8. Layout, keyboard and close out

- [x] 8.1 Lines panel bottom-left above the time bar, card on the right below the mode switch. The
...
- [x] 8.2 At phone width, one column with the lines panel collapsed by default. Verify at 400 px.
- [x] 8.3 Keyboard: reachable stations via the panel rather than a roving focus over a canvas, a
