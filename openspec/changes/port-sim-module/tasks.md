...
- [ ] 1.1 Create `src/sim/types.ts` with the `network.json` shape: `DayType`, `HeadwayWindow`,
      `Origin`, `LonLat`, `Stop`, `Direction`, `Mode`, `Line`, `Station`, `Network`. Verify by
      running `npm run build` — it must type-check with no `any`.
- [ ] 1.2 Add the prepared and result types: `PreparedDirection` (adds `duration` and a
...
- [ ] 1.3 Add a single typed entry point for the imported JSON (the inferred type of
...
- [ ] 2.1 Implement `prepare(net: Network): PreparedNetwork` in `src/sim/prepare.ts`: project each
...
- [ ] 2.2 Expand each direction's headway windows into a sorted list of departure seconds, using
...
- [ ] 2.3 Set each direction's `duration` to its last stop's `dep`. Verify with a test that every
      direction's duration is greater than zero and less than 7200 s.
- [ ] 2.4 Verify `prepare` does not mutate its argument: a test that snapshots
      `JSON.stringify(network)` before and after preparing, and asserts they are identical.

## 3. The simulation core

- [ ] 3.1 Implement `lowerBound(arr, x)` in `src/sim/` — first index where `arr[i] >= x`. Verify
      with a unit test covering an empty array, a value below all elements, above all elements, and
      an exact match on a duplicate.
- [ ] 3.2 Implement `progress(dir, offset)`, keeping the smoothstep easing `f * f * (3 - 2 * f)`
...
- [ ] 3.3 Implement `pointAt(line, at, reversed): Point` — binary-search the segment, interpolate in
      ...
      point differ by 180°.
- [ ] 3.4 Implement `activeTrains(net, nowSec, svcToday, svcYesterday)`, preserving the iteration
...
- [ ] 3.5 Implement `nextDepartures(dir, j, nowSec, svcToday, svcYesterday, count)` **as written in
      ...
      `nextDepartures` case.
- [ ] 3.6 Implement `firstDeparture(net, svc): number | null`, returning `null` where the prototype
      returns `Infinity`. Verify with a test asserting 21600 for `MonFri` on the shipped feed.

## 4. The Kuala Lumpur clock

- [ ] 4.1 Implement `svcOf(dow)` and `klNow(epochMs, override = 'auto')` in `src/sim/clock.ts`,
      ...
      date and `sec`.
- [ ] 4.2 Verify time-zone independence: a test that runs `klNow` for a fixed epoch and asserts the
      result, run under `TZ=America/New_York` as well as the default, proving nothing reads the host
      zone.
- [ ] 4.3 Implement `setTimeOfDay(epochMs, sec): number` as a pure function returning the new epoch.
...
- [ ] 4.4 Verify `override` forces both today and yesterday to the same day type (this is why golden
      case 0 reads `serviceYesterday: "MonFri"`).

## 5. Golden tests

- [ ] 5.1 Write `src/sim/golden.test.ts` loading `data/network.json` and `reference/sim-golden.json`,
...
- [ ] 5.2 Extend it to the two cases that enumerate trains, comparing positionally and rounding
      ...
      trains match.
- [ ] 5.3 Add the `nextDepartures` golden case, resolving `stopId` `KJ14` to its index via
      `findIndex` rather than hardcoding 23. Verify it returns `[77, 317, 557]`.
- [ ] 5.4 Add a purity test: assert `src/sim` source files contain no `Date.now`, no `document`, no
      `window`, and no top-level `let`/`var`. Verify it fails if a violation is introduced.

## 6. Close out

- [ ] 6.1 Run `npm test` and `npm run build`; both must pass with no warnings introduced.
- [ ] 6.2 Update the Status section of `CLAUDE.md` for Milestone 2, and add anything learned about
      the feed to the "What we learned about this feed" section. Verify by reading it back.
- [ ] 6.3 Write `docs/decisions/0002-sim-port-conventions.md` recording the bearing convention flip,
