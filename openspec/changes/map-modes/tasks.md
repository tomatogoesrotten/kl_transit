...
- [ ] 1.1 Write a pure helper in `src/map/` that, given the style's layers and a mode, returns which
...
- [ ] 1.2 Apply it with `map.setLayoutProperty(id, 'visibility', …)`. Do **not** use `map.setStyle`:
...
- [ ] 1.3 Read liberty's original `background` colour once at load and keep it, so city mode can be
...
- [ ] 1.4 Confirm the camera is untouched: a test or a by-eye check that position, zoom, pitch and
      bearing are the same after switching.

## 2. The switch

- [ ] 2.1 Create `src/ui/` and a switch component there. It takes the current mode and a change
...
- [ ] 2.2 Make it a real switch: operable by mouse, touch and keyboard, with its current state
...
- [ ] 2.3 Position it so it does not collide with MapLibre's navigation control or the caption, and
      reads at phone width. Verify at 400 px wide.

## 3. Remembering the choice

- [ ] 3.1 Persist the mode to `localStorage` and read it on mount, defaulting to city. Verify by
      switching, reloading, and seeing the same mode.
- [ ] 3.2 Wrap both the read and the write so a browser that refuses storage — a private window, or
      site data blocked — still works and simply does not remember. Verify by forcing the failure.

## 4. Close out

- [ ] 4.1 Keep the scheduled-not-live statement visible in both modes. Verify by eye in each.
- [ ] 4.2 Run `npm test` and `npm run build`; both must pass with the existing 58 tests green.
...
- [ ] 4.4 Update `CLAUDE.md`'s Status, and open a pull request referencing issue #12 with
      `Closes #12`.
