...
Moving the pointer over a train or a station SHALL identify it briefly, without the viewer having to
click. The pointer SHALL indicate that the thing under it can be selected.

Only trains and stations SHALL answer. The drawn track is not a thing to inspect, and if it answered
it would do so everywhere near a line, leaving nothing else reachable.
